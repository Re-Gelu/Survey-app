...
export const DashboardPollsTable = (props: PageDataWithIp) => {
          ...
          <td>
            <Grid align="center" justify="space-between">
                ...
                <Link href={`/polls/${item.id}`}>
                  <Grid align="center" justify="space-between">
                    <Grid.Col span="content" p={0}>
                      <ActionIcon variant="transparent" className={classes.adapt}>
              ...
              <Grid.Col span="content">
                <ActionIcon
                  title={`Delete poll - ${item.question && item.question}`}
                  color="red"
            ...
            >
              <Button variant="outline" color="red" onClick={() => handleDeletion(item.id)}>
  ...
  return !error ? (
    ...
    <Container size="sm" my="xl" px="xl" py="xl">
      <CustomAlert>Error while loading data</CustomAlert>
    </Container>
  );
};
