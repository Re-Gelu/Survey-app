...
const IndexPage = () => {
  const { data, error, isLoading } = useSWR<{ data: Poll[] }, Error>(
    '/api/polls?offset=0&page_size=1000',
    fetcher
  );
    ...
    <>
              ...
              <Paper shadow="sm" radius="md" p="md" withBorder>
                <Text fz="xl" fw={300}>
                  <Blockquote cite="– Re: Gelu" p={0}>
                    Welcome to our voting app! We have developed this application using advanced
                    technology to provide you with a convenient and secure way to conduct voting.
                  </Blockquote>
                </Text>
              </Paper>
            </Container>
      ...
      {data ? (
        ...
        <Container size="sm" my="xl" px="xl" py="xl">
          <CustomAlert>Error while loading the data!</CustomAlert>
        </Container>
      )}
    </>
  );
...
export const getServerSideProps: GetServerSideProps<Pick<PageDataWithIp, 'fallback'>> = async (
  context
) => {
  ...
  if (nextRequestMeta) {
    ...
    try {
      ...
      const data = await fetcher(
        `${nextRequestMetaData._protocol}://${context.req.headers.host}/api/polls?offset=0&page_size=1000`
      );
