...
const PollPage = (props: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  ...
  const handleError = (errors: typeof form.errors) => {
    if (isPollExpired || isAlreadyVoted) {
      notifications.show({
        message: isPollExpired
          ? 'Time to vote has passed...'
          : isAlreadyVoted && 'You already voted!',
        color: 'red',
  ...
  const handleSubmit = async (
    ...
    axios
      .post(`/api/polls/${router.query.id}/vote`, values)
      .then(() => {
  ...
  useEffect(() => {
    if (!error && !isLoading && typeof data === 'object' && data !== null) {
      votesAmountHandlers.set(
        data.choices.reduce((sum: number, choice: Choice) => sum + choice.votes.length, 0)
      );
      setIsAlreadyVoted(
        data.choices.some((choice: Choice) =>
  ...
  useEffect(() => {
    if (!error && !isLoading && typeof data === 'object' && data !== null) {
      const votedChoice: string[] = data.choices
        .filter((choice: Choice) => choice.votes.some((vote: Vote) => vote.voter_ip === props.ip))
  ...
  useEffect(() => {
    form.setFieldValue('choices', votedOption.length == 1 ? votedOption[0] : votedOption);
  }, [votedOption]);
  ...
  const voting_options =
    data || (!error && !isLoading && data) ? (
      data.is_multiple_answer_options ? (
        ...
        <Checkbox.Group defaultValue={votedOption} {...form.getInputProps('choices')}>
          {data.choices &&
        ...
        <Radio.Group name="choices" {...form.getInputProps('choices')}>
          {data.choices &&
            data.choices.map((item: Choice) => (
              <Radio key={item.text} label={item.text} value={item.text} size="md" mt="lg"></Radio>
            ))}
        </Radio.Group>
      )
    ) : (
            ...
            <Group key={item.text} grow mt="lg">
              ...
              <Transition mounted={isAnimate} keepMounted transition="fade" duration={500}>
                {(styles) => (
                  <Progress
                    style={styles}
                    label={`${item.votes.length}`}
                    size="xl"
                    value={(item.votes.length / votesAmount) * 100}
                  />
                )}
              </Transition>
            </Group>
  ...
  if (error) {
    <Container size="sm" my="xl" px="xl" py="xl">
      <CustomAlert>Such a survey no longer exists</CustomAlert>
    </Container>;
  }
        ...
        <>
          ...
          <Grid my="xl" justify="space-between">
            <Grid.Col sm={6} xs={12}>
              <Title order={2} fw={400}>
                Current poll theme:
                <Text inherit fw={200} variant="gradient" component="span" mx="sm">
            ...
            <Grid.Col span="content">
                  ...
                  {data.expires_at && (
                    <Text component="span" pl={6} fs="italic" fz="lg" c="dimmed">
                      ...
                      {`
                          ...
                          ${new Date(data.expires_at).toLocaleTimeString('ru-RU', {
                            hour: 'numeric',
                            minute: 'numeric',
                            timeZone: 'Europe/Moscow',
                          })}
              ...
              {isPollExpired && (
                <Text fs="italic" fz="xl" c="dimmed" variant="gradient">
                  Poll expired
                </Text>
              )}
            </Grid.Col>
          ...
          <Box component="form" onSubmit={form.onSubmit(handleSubmit, handleError)}>
            ...
            <Group>
              ...
              <Text c="dimmed">{votesAmount} votes</Text>
            </Group>
          </Box>
        </>
      ) : (
  ...
  if (nextRequestMeta) {
    ...
    try {
      ...
      const data = await fetcher(
        `${nextRequestMetaData._protocol}://${context.req.headers.host}/api/polls/${context.query.id}`
      );
...
export default function SWRPrerenderedPage({
  ip,
  fallback,
