...
import { IconCheck, IconExclamationMark, IconSkull, IconTrash } from '@tabler/icons';
...
export const PollCreationForm = () => {
  ...
  const handleError = (errors: typeof form.errors) => {
    ...
    if (
      Object.keys(errors).some(
        (key) => key.startsWith('choices.') && key.endsWith('.text') && errors[key]
      )
    ) {
      notifications.show({
  ...
  const fields = form.values.choices.map((item, index) => (
    <Group key={index} mb="xs">
      ...
      />
      <ActionIcon title={`Remove choice - ${index}`} color="red">
        <IconTrash size="1rem" />
      </ActionIcon>
    </Group>
  ));
  ...
  return (
    <Box maw={500} mx="auto" component="form" onSubmit={form.onSubmit(handleSubmit, handleError)}>
      ...
      <Group position="center" mt="md">
        <Button variant="outline" onClick={() => handleChoiceAdding()}>
          Add poll choice
        </Button>
      </Group>
      ...
      <Group my="md" spacing="xl">
        ...
        />
        <Switch label="Is poll with multiple answers option?" />
      </Group>

      <Group position="center" mt="md">
        <Button type="submit" variant="outline" radius="xl" px="xl">
          Confrim
        </Button>
      </Group>
    </Box>
